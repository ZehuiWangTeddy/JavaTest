...
public class TextItem extends SlideItem {
    ...
    public AttributedString getAttributedString(BasicStyle style, float scale) {
    ...
    // draw the item
    public void draw(int x, int y, float scale, Graphics g, BasicStyle myStyle, ImageObserver o) {
    ...
    public List<TextLayout> getLayouts(Graphics g, BasicStyle s, float scale) {
        List<TextLayout> layouts = new ArrayList<TextLayout>();
        AttributedString attrStr = getAttributedString(s, scale);
        Graphics2D g2d = (Graphics2D) g;
        FontRenderContext frc = g2d.getFontRenderContext();

        // check AttributeString is not null and has a length
        if (attrStr == null || attrStr.getIterator().getEndIndex() == 0) {
            return layouts;
        }
